...
function makeWord(lettersObject) {
  ...
  // Iterate over each position from 0 to maxPosition
  Array.from({ length: maxPosition + 1 }).forEach((_, i) => {
    // Iterate over each letter in lettersObject
    Object.entries(lettersObject).forEach(([letter, pos]) => {
      // If the current position is in the positions array, append the letter to the word
      if (pos.includes(i)) {
        word += letter;
      }
    });
